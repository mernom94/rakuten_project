...
def load_raw_data():
    ...
    features_query = 'SELECT id, designation, description FROM "x_test"'
    X_data = pd.read_sql(features_query, con=engine)
    
    # Load targets
    targets_query = 'SELECT id, prdtypecode FROM "y_test"'
...
def save_processed_data(X_features, y_target, vectorizer, df_processed, data_info):
    ...
    features_path = os.path.join(PROCESSED_DATA_DIR, f'X_features_test.npz')
    from scipy.sparse import save_npz
    save_npz(features_path, X_features)
    
    # Save targets
    targets_path = os.path.join(PROCESSED_DATA_DIR, f'y_target_test.npy')
    np.save(targets_path, y_target.values)
    
    # Save processed text DataFrame (with all text versions)
    text_path = os.path.join(PROCESSED_DATA_DIR, f'processed_text_test.csv')
    df_processed.to_csv(text_path, index=False)
    
    # Save vectorizer
    vectorizer_path = os.path.join(MODELS_DIR, f'vectorizer_test.pkl')
    ...
    metadata_path = os.path.join(PROCESSED_DATA_DIR, f'preprocessing_metadata_test.json')
